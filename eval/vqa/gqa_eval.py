...
class GQAEval:
    def __init__(self, predictions, ques_file_path, choices_path=None, EVAL_CONSISTENCY=False,
                 EVAL_HEAD_TAIL=False, strict=False):
        ##### Files Loading
        ##########################################################################################
        ...
        if EVAL_HEAD_TAIL:
            ...
            """
            self.scores['head_tail'] = [[[], [], []], [[], [], []],
                                        [[], [], []]]  # dim 0: predciction | dim 1: ground truth
            self.qid2reasinfo = {}
        ...
        for qid, question in tqdm(questions.items()):
            ...
            if not self.strict and qid not in self.predictions:
                continue
        ...
        for m, mPrintName in detailedMetrics:
            ...
            for t in sorted(list(self.scores[m].keys())):
                ...
                self.detail_result_string.append(
                    "  {title}: {score:.2f}{suffix} ({amount} questions)".format(title=tName,
